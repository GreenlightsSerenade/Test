...
def st_init(t, nums, start, end, idx):
    if start == end:
        ...
        mid = (start + end) // 2
        t[idx] = st_init(t, nums, start, mid, idx * 2) + st_init(t, nums, mid + 1, end, idx * 2 + 1)
        return t[idx]


def st_update(t, pos, data, start, end, idx):
    if start == end:
        tmp, t[idx] = data - t[idx], data
        loopy = idx // 2
        while loopy > 0:
            t[loopy] += tmp
            loopy //= 2
...
def st_sum(t, rge_s, rge_e, start, end, idx):
    ...
    mid = (start + end) // 2
    if rge_e <= mid:
        return st_sum(t, rge_s, rge_e, start, mid, idx * 2)
    elif rge_s > mid:
        return st_sum(t, rge_s, rge_e, mid + 1, end, idx * 2 + 1)
    else:
        return st_sum(t, rge_s, mid, start, mid, idx * 2) + st_sum(t, mid + 1, rge_e, mid + 1, end, idx * 2 + 1)
